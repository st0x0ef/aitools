...
public class ComputerDisplay extends BasicDisplay {
    public ComputerDisplay(RecipeHolder<ComputerRecipe> recipe) {
        super(getInputList(recipe.value()), List.of(EntryIngredient.of(EntryStacks.of(recipe.value().getResultItem(null)))));
    }

    private static List<EntryIngredient> getInputList(ComputerRecipe recipe) {
        if (recipe == null) {
