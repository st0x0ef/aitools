...
public record AIToolData(Map<ResourceLocation, Integer> blocksBrokenData, int fortuneLevel, int radiusLevel, int efficiencyLevel) implements Serializable {
    public static final Codec<AIToolData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.INT.fieldOf("efficiencyLevel").forGetter(AIToolData::efficiencyLevel)
    ).apply(instance, AIToolData::new));



    public static final StreamCodec<ByteBuf, AIToolData> STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.map(HashMap::new, ResourceLocation.STREAM_CODEC, ByteBufCodecs.VAR_INT), AIToolData::blocksBrokenData, ByteBufCodecs.INT, AIToolData::fortuneLevel, ByteBufCodecs.INT, AIToolData::radiusLevel, ByteBufCodecs.INT, AIToolData::efficiencyLevel, AIToolData::new);
    ...
    public static AIToolData empty() {
        return new AIToolData(new HashMap<>(), 0, 0, 0);
    }
}